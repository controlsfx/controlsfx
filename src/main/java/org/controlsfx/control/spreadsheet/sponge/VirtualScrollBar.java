...
public class VirtualScrollBar extends ScrollBar {
	...
	@Override public void adjustValue(double pos) {
		if (isVirtual()) {
			...
			adjusting = true;
			final double oldValue = flow.getPosition();

			final double newValue = (getMax() - getMin()) * Utils.clamp(0, pos, 1)+getMin();
			if (newValue < oldValue) {
				final IndexedCell cell = flow.getFirstVisibleCell();
				if (cell == null) {
					return;
				}
				flow.showAsLast(cell);
			} else if (newValue > oldValue) {
				final IndexedCell cell = flow.getLastVisibleCell();
				if (cell == null) {
					return;
				}
