...
public class NestedTableColumnHeader extends TableColumnHeader {
	...
	@Override public void setTableHeaderRow(TableHeaderRow header) {
		...
		for (final TableColumnHeader c : getColumnHeaders()) {
	...
	void updateTableColumnHeaders() {
		...
		for (int i = 0; i < getColumnHeaders().size(); i++) {
			final TableColumnHeader header = getColumnHeaders().get(i);
			header.dispose();
		}
		...
		if (getColumns().isEmpty()) {
			...
			final NestedTableColumnHeader parentHeader = getParentHeader();
			if (parentHeader != null) {
				final List<TableColumnHeader> parentColumnHeaders = parentHeader.getColumnHeaders();
				final int index = parentColumnHeaders.indexOf(this);
			...
			final List<TableColumnHeader> newHeaders = new ArrayList<TableColumnHeader>();

			for (int i = 0; i < getColumns().size(); i++) {
				final TableColumnBase<?,?> column = getColumns().get(i);
				if (column == null) {
					continue;
				}
	...
	@Override void dispose() {
		...
		for (int i = 0; i < dragRects.size(); i++) {
			final Rectangle rect = dragRects.get(i);
	...
	private static final EventHandler<MouseEvent> rectMousePressed = new EventHandler<MouseEvent>() {
		@Override public void handle(MouseEvent me) {
			...
			final TableColumnBase column = (TableColumnBase) rect.getProperties().get(TABLE_COLUMN_KEY);
			final NestedTableColumnHeader header = (NestedTableColumnHeader) rect.getProperties().get(TABLE_COLUMN_HEADER_KEY);
			...
			if (me.getClickCount() == 2 && me.isPrimaryButtonDown()) {
				...
				final Rectangle innerRect = (Rectangle) me.getSource();
				final double startX = header.getTableHeaderRow().sceneToLocal(innerRect.localToScene(innerRect.getBoundsInLocal())).getMinX() + 2;
	...
	private void rebuildDragRects() {
		...
		final boolean isConstrainedResize = TableView.CONSTRAINED_RESIZE_POLICY.equals(
	...
	private void columnResizing(TableColumnBase col, MouseEvent me) {
		...
		final double delta = draggedX - lastX;
		final boolean allowed = getTableViewSkin().resizeColumn(col, delta);
		if (allowed) {
			lastX = draggedX;
		}
	}
	...
	@Override protected void layoutChildren() {
		...
		int i = 0;
		for (final TableColumnHeader n : getColumnHeaders()) {
			if (! n.isVisible()) {
				continue;
			}
	...
	private TableColumnHeader createColumnHeader(TableColumnBase col) {
		final TableColumnHeader newCol = createTableColumnHeader(col);
