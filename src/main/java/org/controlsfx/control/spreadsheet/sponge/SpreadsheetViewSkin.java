...
public class SpreadsheetViewSkin<T> extends SpreadsheetViewSkinBase<T, TableView<T>, TableViewBehavior<T>, TableRow<T>> {

	private final TableView<T> tableView;


	public SpreadsheetViewSkin(final TableView tableView, final SpreadsheetView spreadsheetView) {
		...
		spreadsheetView.setHbar(flow.getHbar());
		spreadsheetView.setVbar(flow.getVbar());
		final SpreadsheetView.RowAccessor<TableRow<T>> lcells = new SpreadsheetView.RowAccessor<TableRow<T>>() {
			...
			public TableRow<T> get(int index) {
				return flow.cells.get(index);
			}

			@Override
			public boolean isEmpty() {
				return flow.cells.isEmpty();
			}
		};
		...
		public void onChanged(Change<? extends Integer> c) {
			if(flow.getFixedColumns().size() > c.getList().size()){
				for(int i=0;i<flow.cells.size();++i){
					((SpreadsheetRow) flow.cells.get(i)).putFixedColumnToBack();
				}
			}
